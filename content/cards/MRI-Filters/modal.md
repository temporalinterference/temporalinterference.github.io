---
id: MRI-Filters
---
# TI-MRI Filter Solutions

## TI-MRI Filter Solutions of the IT'IS Foundation
The unique TI-MRI Filter Solutions of the [IT'IS Foundation](https://itis.swiss) allow for artifact-free concurrent functional magnetic resonance imaging (fMRI) recording during temporal interference (TI) stimulation with our TIBS-R system.  

The TI protocol is conveniently operated from the PC in the MRI control room, where the TIBS-R is also located. The standard Electrode Connection Box (ECB) is replaced with an fMRI-compatible version supplied by IT’IS.

An additional breakthrough is the set of leads developed by TI Solutions in collaboration with [EMPA](https://www.empa.ch/web/empa/), Switzerland, featuring optimized distributed resistance for maximum safety and artifact-free recording during fMRI. TIBS-R supports up to four stimulation channels (eight electrodes).

---

## Specifications of TI-MRI Filter Solutions
### MRI Filters for TIBS-R

|    |            |
|:-----------|:------------|
|Number of Stimulation Channels|4 (8 electrodes) |
|Electrode Connector Type|Touch-Proof 1.5 mm female |
|Default Feedthrough-Filter Mounting Option|20 mm clearance hole |
|Alternative Feedthrough-Filter Mounting Option|Insertion into waveguide padded with metallic wool, filter diameter 40 mm |
|RF Attenuation| >70 dB (both differential and common mode) |
|Cable Length Inside Shield Room|10 m |
|Cable Length Outside Shield Room|2.9 m |
|Number of Cable Traps|4 (2.9 T and 3.0 T)  3 (7.0 T) |
|MR Operating Modes|Normal operating mode only |
|Validated Sequences|High specific absorption rate, high gradient |

---
### MRI-Optimized Electrode Lead Set

|    |            |
|:-----------|:------------|
|Number of Electrodes per Set|10 |
|Electrode Type|Ag/AgCl; others upon request |
|Electrode Lead Type|Novel lead technology with distributed resistance of 3+/-10%&nbsp;kOhm/m warranting safe and artifact-free MRI scans |
|Electrode Lead Length|60 / 80 cm |
|Electrode Connector Type|Touch-Proof 1.5 mm male |

---

{{< modal-image TI-MRI-Filter-Solutions-1500x1500px.jpg >}}
{{< /modal-image >}}
cMRI3.0T-TIBS-R

---

## Available Systems

|    |            |            |
|:-----------|:------------|:------------|
|**Part Number**|**Operating Frequency**|**Scanner Manufacturer** |
|TIBS-R-I-MRI-2.9T|123.2 MHz|Siemens |
|TIBS-R-I-MRI-3.0T|127.8 MHz|Philips, GE, Canon |
|TIBS-R-I-MRI-7.0T|298 MHz|Siemens, GE, Philips |

---

## Disclaimer

The TI-MRI Filter Solutions are third-party tools developed by the [IT'IS Foundation](https://itis.swiss). We do not control or endorse these third-party tools. We do not assume any responsibility or give any assurance for the accuracy, functionality, availability, usability, applicability, or performance of the tools provided by IT'IS and the results generated by these third-party tools, nor do we assume any responsibility for any potential issues or damages resulting from their use.







